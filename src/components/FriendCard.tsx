...
const FriendCard = ({user, navigation}: FriendCardProps) => {
  ...
  <MaterialCommunityIcons
        onPress={() => {
            ...
            if (curr?.competition && Object.keys(curr.competition).length > 0 && curr.competition.competitor === friendUid) {
            firebase.firestore().collection('users').doc(curr.competition.competitor).update({competition: {}});
         ...
         firebase.firestore().collection('users').doc(friendUid).get().then((snapshot) => {
              ...
              const friendCompRequests = friend.competitionRequests.filter((item) => item.uid !== uid);
              firebase.firestore().collection('users').doc(friendUid).update({competitionRequests: friendCompRequests});
            });

          const compRequests = curr?.competitionRequests.filter((item) => item.uid !== friendUid);
            firebase.firestore().collection('users').doc(uid as string).update({competitionRequests: compRequests});

        const habitRef = firebase.firestore()
          .collection('habits')
          .where('friends', 'array-contains', uid).where('user', '==', friendUid);
        ...
        const myHabitRef = firebase.firestore()
          .collection('habits')
          .where('friends', 'array-contains', friendUid).where('user', '==', uid);
